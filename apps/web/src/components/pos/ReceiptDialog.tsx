...
export function ReceiptDialog({ sale, onClose }: { sale: PosSaleResult; onClose: () => void }) {
  ...
  async function handleShare() {
    ...
    setNotice(null);
    try {
      const number = sale.receiptNumber ?? sale.invoiceNumber ?? sale.id.slice(0, 8);
      ...
      >
        <div className="flex items-center justify-between border-b px-4 py-3 print:hidden" style={{ borderColor: 'var(--color-border)' }}>
          ...
          <div className="flex items-center gap-1">
            <button type="button" disabled={sharing} onClick={handleShare} className="flex h-8 w-8 items-center justify-center rounded-md disabled:opacity-50" style={{ color: 'var(--color-ink-600)' }} aria-label="Share">
        ...
        {notice && (
          <p className="px-4 pt-3 text-center text-xs print:hidden" style={{ color: 'var(--color-ink-600)' }}>
            {notice}
          </p>
        )}

        <div id="print-area" className="print-thermal overflow-y-auto p-5 font-mono text-sm" style={{ color: 'var(--color-ink-900)' }}>
          ...
          {/* Every line is qty × unit price = line total — the unit price already IS
              whatever was negotiated for this sale. No separate "discount" figure. */}
          <div className="flex flex-col gap-2">
            {sale.items.map((item) => (
              <div key={item.id} className="flex justify-between gap-2">
                ...
                <span className="shrink-0 data-num">{money(item.lineTotal)}</span>
              </div>
            ))}
          </div>
